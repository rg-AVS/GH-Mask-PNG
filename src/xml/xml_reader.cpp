...
void XmlElement::setAttrD(const std::string& name, double value, int precision) {
    ...
    if (precision <= 0) {
        // precision 0 is how callers say "this attribute is an integer"
        // (xres, yres, hvmix, ...). It must NOT go through the default float
        // format, which honours precision as SIGNIFICANT DIGITS and would
        // write 1024 as "1e+03" -- reparsing as 1000, silently resizing every
        // mask that round-trips through the writer.
        ss << (long long)std::llround(value);
    } else {
